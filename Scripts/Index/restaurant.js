...
export function initRestaurants(data) {
  createRestaurantCards(data) // Crea las tarjetas iniciales
  getRandomRestaurant(data) // Muestra el restaurante aleatorio por defecto

  // Agregar eventos al botón de restaurante aleatorio
  document.getElementById("select-random-btn")?.addEventListener("click", () => getRandomRestaurant(data))

  // Agregar eventos al botón de recargar tarjetas
  document.getElementById("reload-restaurants-btn")?.addEventListener("click", () => createRestaurantCards(data))
...
function createRestaurantCards(data) {
  ...
  // Filtrar restaurantes de bajo costo
  const filtered = data.filter((r) => r.price_level === "$")
...
function getRandomRestaurant(data) {
  ...
  const cuisineTags = (r.cuisine_types?.split(", ") || [])
    .map((c) => `<span class="cuisine-tag" onclick="navigateToCuisine('${c}')">${c}</span>`)
    .join("")

  const address = r.address_obj?.street1 || "Dirección no disponible"
  ...
  box.innerHTML = `
    ...
    <div class="random-restaurant-content">
      ...
      <p class="mb-2">${displayPriceLevel(r.price_level)}</p>
